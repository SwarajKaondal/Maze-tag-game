...
function loadMinimap() {
  ...
  minimap.fillRect(size * (self[0] + 1), size * (self[1] + 1), size, size);
  if (currentEnemyPosition != undefined) {
    let enemy = [
      Math.floor(currentEnemyPosition.position.Eye[0] / blockLength),
      Math.floor(currentEnemyPosition.position.Eye[2] / blockLength),
    ];
    minimap.fillStyle = "#ff0000";
    minimap.fillRect(size * (enemy[0] + 1), size * (enemy[1] + 1), size, size);
  }
}
